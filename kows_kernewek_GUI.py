...
if __name__ == '__main__':
    ...
    # output display
    msg2 = tk.Label(root)
    msg2.config(bg = 'light yellow', fg = 'dark red', font=('Open Sans', 18, 'bold'))
    msg2.pack(expand=tk.YES,fill=tk.BOTH, anchor=tk.CENTER)

    # buttons
    Kwitya(root).pack(side=tk.RIGHT)
    tk.Button(root, text = 'Kewsel', font=('Open Sans',14),           
           command = printentbar).pack(side=tk.RIGHT)
    tk.Button(root, text = 'Gorhemmyn', font=('Open Sans',14),
